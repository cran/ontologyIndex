...
#' @template ontology
#' @template roots
#' @param exclude_roots Boolean determining whether to remove the given \code{roots} terms from the result.
...
get_descendants <- function(ontology, roots, exclude_roots=FALSE) {
	...
	if (exclude_roots) setdiff(result, roots) else result 
}
...
intersection_with_descendants <- function(ontology, roots, terms) as.character("if"(
	length(terms) > 0,
	terms[
		sapply(
...
exclude_descendants <- function(ontology, roots, terms) as.character(Filter(
	x=terms,
